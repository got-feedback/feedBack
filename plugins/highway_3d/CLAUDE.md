...
`tuning` and `capo` feed only the nut's open-string pitch labels (`_openStringPitchLabelsForTuning`), which prefer `bundle.tuning`/`bundle.capo` over `songInfo` — the bundle fields are chart-transform-aware (a chart-transform provider substitutes the target instrument there), while `getSongInfo()` keeps the chart's originals by contract. Note placement never reads them.
